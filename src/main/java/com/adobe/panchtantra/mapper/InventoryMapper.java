...
public class InventoryMapper {
    ...
    public Inventories convertListOfInventoryToInventories(List<Inventory> listOfInventory) {
        if(CollectionUtils.isEmpty(listOfInventory)) {
            return null;
        }
        Inventories inventories = new Inventories();
        listOfInventory.stream().forEach(inventory -> inventories.add(convertInventoryToInventoryDto(inventory)));
        return inventories;
    }

    private com.adobe.panchtantra.model.Inventory convertInventoryToInventoryDto(Inventory inventory) {
        if(inventory == null) {
            return null;
        }
